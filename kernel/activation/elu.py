...
for (S, K) in SKs:
    ...
    run_benchmark(torch_elu,     "f32_th", x, y_torch)
    run_benchmark(lib.elu_f32,   "f32",    x, y, check_diff, diff, y_torch)
    run_benchmark(lib.elu_f32x4, "f32x4",  x, y, check_diff, diff, y_torch)

    print("-" * 50)
    x_f16 = x.half().contiguous()
    y_f16 = y.half().contiguous()
    y_f16_torch = y.half().contiguous()
    run_benchmark(torch_elu,          "f16_th",    x_f16, y_f16_torch)
    run_benchmark(lib.elu_f16,        "f16",       x_f16, y_f16, check_diff, diff, y_f16_torch)
    run_benchmark(lib.elu_f16x2,      "f16x2",     x_f16, y_f16, check_diff, diff, y_f16_torch)
    run_benchmark(lib.elu_f16x8,      "f16x8",     x_f16, y_f16, check_diff, diff, y_f16_torch)
    run_benchmark(lib.elu_f16x8_pack, "f16x8pack", x_f16, y_f16, check_diff, diff, y_f16_torch)
